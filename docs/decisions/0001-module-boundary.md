# ADR-001: Keep transport-local as an Independent Gnalloy Module
...
`transport-local` is published as `gnalloy.org/transport-local` and owns this responsibility: In-process local paired-channel transport for Gnalloy tests and embedded deployments.
