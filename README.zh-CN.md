...
面向 Gnalloy 测试和嵌入式部署的进程内 paired-channel local 传输。

该模块负责一个具体 I/O 边界，为具体传输创建或适配 Gnalloy Channel。协议解析、业务 handler、TLS 策略和观测能力由其他模块负责。

## 状态

- 导入路径：`gnalloy.org/transport-local`
- 仓库：`github.com/gnalloy/transport-local`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/transport-local@dev`
- 许可证：Apache-2.0
...
本仓库负责：面向 Gnalloy 测试和嵌入式部署的进程内 paired-channel local 传输。
